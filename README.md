...
The recurring commitment auto-deduct flow and offline queue are best presented as experimental extensions unless the target demo database has the required tables and columns configured.
...
10. The Settings view contains email report history, report preferences, recurring commitments, wage-based life-energy settings, and onboarding controls.
...
- The extension field `net_hourly_wage` must exist in `report_preferences` before that setting is fully persistent.
...
- Promote recurring commitments from experimental to fully documented production features after migrations and final QA.
